...
function renderConsultativeOpening(play) {
  ...
  const copy = {
    pt: { themes: { ai: 'produtividade, inteligência artificial e adoção responsável', security: 'segurança, proteção, riscos e continuidade do negócio', cloud: 'modernização, infraestrutura, escalabilidade e nuvem', automation: 'automação, eficiência operacional e inovação low-code', data: 'dados, governança, conformidade e proteção da informação', general: 'produtividade, segurança, automação e transformação digital' }, before: 'Antes da primeira pergunta', title: 'Abertura consultiva', intro: 'Use este roteiro como apoio. Fale de forma natural e adapte o tom à conversa, sem começar diretamente pelo produto.', done: '✓ Abertura realizada', required: 'Etapa obrigatória', copyButton: 'Copiar roteiro', confirm: 'Confirme os elementos realizados', checklist: ['Saudação', 'Apresentação do CLM', 'Motivo da reunião', 'Geração de contexto', 'Explicação do papel consultivo', 'Pedido de permissão para iniciar'], registered: 'Abertura registrada por', continue: 'Ir para Etapa 1 — Contexto do negócio →', complete: 'Abertura realizada ✓', help: 'Confirme todos os itens para liberar a Etapa 1 — Contexto do negócio.', copied: 'Roteiro copiado.', success: 'Abertura registrada. Etapa 1 liberada.' },
    en: { themes: { ai: 'productivity, artificial intelligence, and responsible adoption', security: 'security, protection, risk, and business continuity', cloud: 'modernization, infrastructure, scalability, and cloud', automation: 'automation, operational efficiency, and low-code innovation', data: 'data, governance, compliance, and information protection', general: 'productivity, security, automation, and digital transformation' }, before: 'Before the first question', title: 'Consultative opening', intro: 'Use this script as a guide. Speak naturally and adapt the tone to the conversation without starting directly with the product.', done: '✓ Opening completed', required: 'Required step', copyButton: 'Copy script', confirm: 'Confirm the completed elements', checklist: ['Greeting', 'CLM introduction', 'Reason for the meeting', 'Context setting', 'Explanation of the consultative role', 'Permission to begin'], registered: 'Opening recorded by', continue: 'Go to Step 1 — Business context →', complete: 'Opening completed ✓', help: 'Confirm every item to unlock Step 1 — Business context.', copied: 'Script copied.', success: 'Opening recorded. Step 1 unlocked.' },
    es: { themes: { ai: 'productividad, inteligencia artificial y adopción responsable', security: 'seguridad, protección, riesgos y continuidad del negocio', cloud: 'modernización, infraestructura, escalabilidad y nube', automation: 'automatización, eficiencia operativa e innovación low-code', data: 'datos, gobernanza, cumplimiento y protección de la información', general: 'productividad, seguridad, automatización y transformación digital' }, before: 'Antes de la primera pregunta', title: 'Apertura consultiva', intro: 'Utilice este guion como apoyo. Hable de forma natural y adapte el tono a la conversación sin comenzar directamente por el producto.', done: '✓ Apertura realizada', required: 'Etapa obligatoria', copyButton: 'Copiar guion', confirm: 'Confirme los elementos realizados', checklist: ['Saludo', 'Presentación del CLM', 'Motivo de la reunión', 'Generación de contexto', 'Explicación del papel consultivo', 'Permiso para comenzar'], registered: 'Apertura registrada por', continue: 'Ir a la Etapa 1 — Contexto del negocio →', complete: 'Apertura realizada ✓', help: 'Confirme todos los elementos para habilitar la Etapa 1 — Contexto del negocio.', copied: 'Guion copiado.', success: 'Apertura registrada. Etapa 1 habilitada.' }
  }[language] || null;
  ...
  document.getElementById('discoveryQuestionList').innerHTML = `<section class="consultative-opening ${completed ? 'is-complete' : ''}"><div class="opening-heading"><div><p class="eyebrow">${copy.before}</p><h3>${copy.title}</h3><p>${copy.intro}</p></div><span class="opening-status">${completed ? copy.done : copy.required}</span></div><div class="opening-script"><pre id="consultativeOpeningScript">${escapeHtml(script)}</pre><button id="copyOpeningScriptButton" type="button" class="secondary-button">${copy.copyButton}</button></div><fieldset class="opening-checklist" ${completed || state.readOnly ? 'disabled' : ''}><legend>${copy.confirm}</legend>${checklist.map((item, index) => `<label><input type="checkbox" data-opening-check="${index}" ${completed ? 'checked' : ''}><span>✓</span>${item}</label>`).join('')}</fieldset>${completed ? `<div class="opening-complete-note"><strong>${copy.registered} ${escapeHtml(meeting.abertura_consultiva.clm || USER_NAME)}</strong><span>${new Date(meeting.abertura_consultiva.realizada_em).toLocaleString(appLocale())} · ${escapeHtml(meeting.abertura_consultiva.sales_play || play.titulo)}</span></div><button id="continueAfterOpeningButton" type="button" class="primary-button">${copy.continue}</button>` : `<button id="completeOpeningButton" type="button" class="primary-button opening-complete-button" disabled>${copy.complete}</button><p class="opening-help">${copy.help}</p>`}</section>`;
  ...
  if (completed) {
    document.getElementById('continueAfterOpeningButton')?.addEventListener('click', () => { activeDiscoveryStage = 'situacao'; renderDiscovery(); });
    return;
  }
  const checks = [...document.querySelectorAll('[data-opening-check]')];
  const completeButton = document.getElementById('completeOpeningButton');
  checks.forEach((check) => check.addEventListener('change', () => { completeButton.disabled = !checks.every((item) => item.checked); }));
  completeButton?.addEventListener('click', () => {
    if (!checks.every((item) => item.checked) || state.readOnly) return;
    const currentMeeting = getMeetingById(state.activeMeetingId);
    currentMeeting.abertura_consultiva = { concluida: true, realizada_em: new Date().toISOString(), clm: USER_NAME, sales_play_id: play.id, sales_play: play.titulo, itens: checklist };
    currentMeeting.etapa_discovery_ativa = 'situacao';
    saveMeeting(currentMeeting);
    registrarAuditoria(state.activeMeetingId, `Abertura consultiva realizada por ${USER_NAME} para o Sales Play ${play.titulo}.`);
