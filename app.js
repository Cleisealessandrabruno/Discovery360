...
function navigate(view, meetingId = null) {
  ...
  if (view === 'workspace') openWorkspace(meetingId);
...
function saveFromForm(status) { const meeting = collectMeetingForm(); const required = ['cliente', 'empresa', 'data', 'assunto']; const missing = required.filter((field) => !meeting[field]); document.querySelectorAll('#meetingForm input, #meetingForm textarea').forEach((field) => field.classList.remove('invalid')); if (missing.length) { missing.forEach((field) => document.querySelector(`[name="${field}"]`).classList.add('invalid')); document.getElementById('formError').textContent = 'Preencha os campos obrigatórios destacados.'; return; } const wasNew = !state.editingMeetingId; meeting.status = status; const saved = saveMeeting(meeting); if (wasNew) registrarAuditoria(saved.id, status === 'em_andamento' ? 'Reunião criada e iniciada.' : 'Reunião criada como rascunho.'); else if (status === 'em_andamento') registrarAuditoria(saved.id, 'Reunião iniciada.'); if (status === 'em_andamento') navigate('workspace', saved.id); else navigate('dashboard'); }
...
function updateAccessNotification() { const badge = document.getElementById('accessNotificationBadge'); const count = getAccessRequests().filter((request) => request.status === 'pendente').length; if (badge) { badge.textContent = count; badge.hidden = !isAdmin() || count === 0; } const nav = document.getElementById('accessNav'); if (nav) nav.setAttribute('aria-label', count ? `Validação de acessos: ${count} solicitação${count === 1 ? '' : 'ões'} pendente${count === 1 ? '' : 's'}` : 'Validação de acessos'); return count; }
